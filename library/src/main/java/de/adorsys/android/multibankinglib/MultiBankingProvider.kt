...
class MultiBankingProvider : ContentProvider() {

    override fun onCreate(): Boolean {
        MultiBankingProvider.context = WeakReference(context)
        return true
    }
    ...
    override fun attachInfo(context: Context, providerInfo: ProviderInfo?) {
        if (providerInfo == null) {
            throw NullPointerException("MultiBankingProvider ProviderInfo cannot be null.")
        }
        // So if the authorities equal the library internal ones, the developer forgot to set his applicationId
        if ("<your-library-applicationid>.de.adorsys.android.multibankinglib" == providerInfo.authority) {
